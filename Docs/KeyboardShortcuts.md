# 键盘快捷键参考

> 按 ESC 关闭

### 主界面搜索/浏览
- 浏览列表项目 | 上下光标 | 在主列表中导航
- 跳到第一项/最后一项 | 行首/行尾 | 在主列表中快速跳到第一项/最后一项
- 激活列表项目 | 回车 | 在列表中的项目上按回车
- 执行搜索 | 回车 | 在搜索框或类型组合框内按回车
- 后退到上一视图 | 退格 | 返回上一页/主页
- 听歌识曲 | Ctrl + Shift + L | 打开听歌识曲对话框，选择聆听设备后开始识别
- 收藏聚焦项目（若支持） | Ctrl + L | 仅当当前聚焦项目支持收藏且已登录时生效，不支持或未登录时不执行任何操作
- 取消收藏聚焦项目（若支持） | Ctrl + U | 仅当当前聚焦项目支持取消收藏且已登录时生效，不支持或未登录时不执行任何操作
- 添加到歌单（若支持） | Ctrl + A | 仅当当前聚焦项目为可添加的歌曲且已登录时生效，不支持或未登录时不执行任何操作
- 插播（若支持） | Ctrl + I | 仅当当前聚焦项目支持插播时生效，不支持时不执行任何操作
- 分享（若支持） | Ctrl + S | 仅当当前聚焦项目支持分享时生效，触发后直接进入该项目的分享子菜单
- 评论（若支持） | Ctrl + C | 仅当当前聚焦项目支持评论时生效，触发后直接打开评论窗口
- 下载 | CTRL+回车 | 激活当前聚焦项的下载入口
- 调整顺序 | Ctrl + Alt + 上/下 | 在“创建和收藏的歌单”或自建歌单歌曲列表中上下移动当前项

### 主界面播放/控制
- 播放 / 暂停 | 空格 | 切换播放状态
- 上一曲 / 下一曲 | F3 / F4 | 在播放队列中切换歌曲
- 循环切换播放次序 | Ctrl + R | 按“顺序播放 -> 列表循环 -> 单曲循环 -> 随机播放”循环切换，并播报当前次序
- 音量减 / 音量加 | F1 / F2 | 以 2% 步长调节音量
- 快退 / 快进 | 左右光标 | 以 5 秒为步长调整进度
- 切换输出设备 | F9 | 在当前可用的声音输出设备之间切换
- 切换歌词朗读 | F11 | 启用或关闭屏幕阅读器的歌词朗读
- 切换序号显示 | F8 | 隐藏或显示列表序号
- 切换控制栏显示 | F7 | 隐藏或显示控制栏组件
- 任意位置跳转 | F12 | 打开跳转对话框，接受 [时：分：秒] 或百分比%格式，完成输入后回车跳转
- 刷新 | F5 | 重新加载当前列表/页面
- 隐藏到托盘 | Shift + Esc | 最小化到托盘并保持后台播放

### 评论
- 展开 | 右光标 | 展开当前节点的楼层回复
- 折叠 | 左光标 | 折叠当前节点的楼层回复
- 回复选中评论 | Enter | 在评论树中按回车可直接打开回复对话框
- 复制评论文本 | Ctrl + C | 复制当前选中评论的完整内容
- 删除自己的评论 | Delete | 仅可用于自己发表的评论，删除前将二次确认
- 发表评论 / 回复 | Shift + Enter | 在评论或回复输入框中按 Shift + Enter 发送，单独按 Enter 可换行
- 刷新 | F5 | 重新加载当前评论树视图
- 切换序号的屏幕阅读器输出 | F8 | 省略或读出列表序号

## Buttons
- [关闭](action:close)
