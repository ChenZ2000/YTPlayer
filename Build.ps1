...
Write-Host "  构建 YTPlayer - Release 版本" -ForegroundColor Cyan
Write-Host ("─" * 75) -ForegroundColor Cyan
Write-Host ""
...
& $msbuildPath /t:Restore $solutionFile /p:Configuration=Release /p:Platform="x64" /v:minimal /nologo
...
# 构建项目（Release 配置）
Write-Host "[2/2] 开始编译项目 (Release 配置, x64)..." -ForegroundColor Yellow
Write-Host ""

$startTime = Get-Date
& $msbuildPath $solutionFile /p:Configuration=Release /p:Platform="x64" /t:Rebuild /v:minimal /nologo /m
...
if ($exitCode -eq 0) {
    ...
    if (Test-Path "bin\Release\YTPlayer.exe") {
        $fileInfo = Get-Item "bin\Release\YTPlayer.exe"
        Write-Host "  ✓ bin\Release\YTPlayer.exe (启动器)" -ForegroundColor Green
    ...
    if (Test-Path "bin\Release\YTPlayer.Updater.exe") {
        $fileInfo = Get-Item "bin\Release\YTPlayer.Updater.exe"
        Write-Host "  ✓ bin\Release\YTPlayer.Updater.exe" -ForegroundColor Green
        Write-Host "    大小: $([math]::Round($fileInfo.Length / 1KB, 1)) KB" -ForegroundColor Gray
        Write-Host "    修改时间: $($fileInfo.LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss'))" -ForegroundColor Gray
    }
    ...
    Write-Host "  .\bin\Release\YTPlayer.exe" -ForegroundColor White
    Write-Host ""
} else {
