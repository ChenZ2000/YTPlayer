...
- 一切主页入口进入浏览到的歌单、歌手和专辑的歌曲总数都始终为 0 ，歌手搜索结果列表的歌手歌曲总数标签也始终为 0 ，这些位置的总数获取逻辑需要修复。如果确定某些总数获取逻辑在参考项目中找不到或无法开发获取逻辑,则将这个总数标签隐藏不在列表项上显示。类似的，对`未知歌手`和`未知专辑`这类返回空数据/占位符的情况，也同样直接隐藏标签即可，让 UI 和屏幕阅读器输出更简洁。
...
- 歌手的专辑分页显示逻辑中每页条目数也太少，我希望从当前的 30 张也调整为 100 张，对齐搜索结果列表的逻辑。
...
- 主页，尤其是登录状态下，有时会在启动后/从文件/操作菜单激活后一直加载不出来，且加载速度慢。有时需要多次从文件/操作进行主页入口的激活才能最终加载主页。[`D:\GitHub\YTPlayer\bin\Debug\Logs\Debug_20251105_072127.log`]
