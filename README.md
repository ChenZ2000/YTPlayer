...
- 主页提供“最近听过”入口，集中展示最近歌曲/歌单/专辑的动态总数（启动、返回主页或再次激活“主页”命令时都会刷新）。
- 支持播客/电台搜索、节目浏览与收藏，进入播客详情后自动加入历史导航、上下文菜单和下载工作流。
- 搜索框支持直接粘贴网易云歌曲 / 歌单 / 专辑 / 歌手网页 URL，自动切换到相应分类并加载对应入口。
- 提供二维码与短信验证码登录。
- 云盘浏览、上传、下载与删除。
...
- 播客支持单期节目下载与整台批量下载，和歌曲一样可参与批量下载/目录自定义并复用已有的下载管理器。
