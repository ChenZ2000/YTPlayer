...
### 搜索和浏览
- 支持歌曲 / 歌手 / 专辑 / 歌单多类型浏览和搜索、收藏/取消收藏、创建新歌单。
- 全局支持 Backspace 返回和 F5 刷新。
...
- 文件/操作菜单下动态隐藏的“当前播放”入口，可随时对正在播放的歌曲执行相关操作。
...
### 歌曲解封/解灰
- 遇到未登录/账号无 VIP 、歌曲在网易云音乐官方无资源/已下架时，应用会自动尝试查找和匹配替代资源并播放。（不保证百分百命中率）
解封/解灰逻辑同样接入歌曲/歌词下载和直链分享
